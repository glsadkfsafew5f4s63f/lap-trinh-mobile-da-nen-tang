import type { ApiProduct, ApiVariant } from '../services/api';
import { getApiProductVariants, getApiProducts } from '../services/api';
...
export function hydrateProducts(nextProducts: Product[]) {
  products = nextProducts;
}

export async function loadProductsFromApi(): Promise<Product[]> {
  try {
    const apiProducts: ApiProduct[] = await getApiProducts();
    const hydrated = await Promise.all(
      apiProducts.map(async (item: ApiProduct): Promise<Product> => {
        const fallback = getProductById(String(item.id));
        const variants: ApiVariant[] = await getApiProductVariants(item.id);
        const colors: ProductColor[] = Array.from(
          new Map(
            variants.map((variant: ApiVariant) => [
              variant.color,
              { name: variant.color, hex: variant.hex ?? '#D9D9D9' },
            ])
          ).values()
        );
        const sizes: string[] = Array.from(new Set(variants.map((variant: ApiVariant) => variant.size)));

        const images = item.images?.length
          ? item.images.map((image) => image.url)
          : fallback?.images ?? [fallback?.image ?? ''];

        return createSqlProduct({
          id: String(item.id),
          sqlId: item.id,
          name: item.name,
          category: fallback?.category ?? (item.category || 'Áo'),
          brand: fallback?.brand ?? (item.brand || 'FASHION'),
          description: item.description || 'Sản phẩm thời trang chất lượng.',
          price: Number(item.price) || 0,
          oldPrice: Number(item.oldPrice ?? item.price) || Number(item.price) || 0,
          images,
          colors,
          sizes,
          variants: variants.map((variant: ApiVariant) => ({
            id: variant.id,
            color: variant.color,
            size: variant.size,
            sku: variant.sku,
    ...
    if (hydrated.length > 0) {
      hydrateProducts(hydrated);
      return hydrated;
    }
  } catch (error) {
    console.warn('Không tải được sản phẩm từ API, giữ dữ liệu mock.', error);
  }
