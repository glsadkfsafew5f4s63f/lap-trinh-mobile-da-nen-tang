...
exports.update = async (req, res) => {
    try {
        ...
        if (req.body.TrangThai && req.body.TrangThai !== current.TrangThai
            && !ORDER_TRANSITIONS[current.TrangThai]?.includes(req.body.TrangThai)) {
            return res.status(409).json({ success: false, message: 'Không thể chuyển đơn hàng sang trạng thái này.' });
        }
        ...
        if (updatedOrder) {
            realtime.broadcast({ type: 'order.updated', order: updatedOrder });
        }
