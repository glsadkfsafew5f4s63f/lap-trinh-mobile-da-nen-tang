...
exports.getAll = async (req, res) => {
    try {
        const data = await Lichsudonhang.getAll();
...
exports.getById = async (req, res) => {
    try {
        const data = await Lichsudonhang.getById(req.params.id);
...
exports.create = async (req, res) => {
    try {
        const result = await Lichsudonhang.create(req.body);
...
exports.update = async (req, res) => {
    try {
        const result = await Lichsudonhang.update(req.params.id, req.body);
...
exports.remove = async (req, res) => {
    try {
        const result = await Lichsudonhang.delete(req.params.id);
        if (!result.affectedRows) return res.status(404).json({ success: false, message: 'Không tìm thấy dữ liệu' });
...
exports.search = async (req, res) => {
    try {
        ...
        const data = await Lichsudonhang.search(keyword);
        res.json({ success: true, data });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
