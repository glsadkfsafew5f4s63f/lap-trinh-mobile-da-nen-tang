...
Nguoidung.search = async (keyword) => {
    const sql = 'SELECT * FROM `nguoidung` WHERE `TenDangNhap` LIKE ? OR `MatKhau` LIKE ? OR `HoTen` LIKE ? OR `Email` LIKE ? OR `DienThoai` LIKE ? OR `DiaChi` LIKE ? OR `AnhDaiDien` LIKE ?';
    const paramsValue = [keyword, keyword, keyword, keyword, keyword, keyword, keyword];
